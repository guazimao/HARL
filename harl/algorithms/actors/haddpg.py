...
class HADDPG(OffPolicyBase):
    ...
    def get_target_actions(self, obs, available_actions=None):
        ...
        if self.action_type == "Box":
            ...
            actions = gumbel_softmax(logits, hard=True, device=self.device)  # onehot actions for critic training
        elif self.action_type == "MultiDiscrete":
            logits = self.target_actor.get_logits(obs, available_actions)
            actions = []
            for logit in logits:
                action = gumbel_softmax(logit, hard=True, device=self.device)  # onehot actions for critic training
                actions.append(action)
            actions = torch.cat(actions, dim=-1)
        return actions
